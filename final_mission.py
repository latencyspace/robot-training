...
def rotate():
    GPIO.output(AIN1, GPIO.HIGH)
    GPIO.output(AIN2, GPIO.LOW)
    pwm_a.ChangeDutyCycle(16)

    GPIO.output(BIN1, GPIO.LOW)
    GPIO.output(BIN2, GPIO.HIGH)
    pwm_b.ChangeDutyCycle(16)
...
while True:
    ...
    if ids is not None:
        for i in range(len(ids)):
            if marker_sequence[current_target_index] in ids:
                ...
                if z > 7:
                    ...
                    stop()
                    if marker_sequence[current_target_index] in [1, 4]:
                        buzz()
                        sleep(1)  # 잠시 멈춤
